...
namespace mob {
    ...
    void dump_stacktrace(const wchar_t* what)
    {
        ...
        const std::size_t frame_count =
            CaptureStackBackTrace(0, max_frames, frame_addresses, nullptr);
        ...
        for (std::size_t i = 0; i < frame_count; ++i) {
            ...
            if (SymGetLineFromAddrW64(process,
                                      reinterpret_cast<DWORD64>(frame_addresses[i]),
                                      &disp, &line)) {
            ...
            if (SymFromAddrW(process, reinterpret_cast<DWORD64>(frame_addresses[i]),
                             &disp2, sym)) {
    ...
    void terminate_handler() noexcept
    {
        try {
            ...
            const int n = _snwprintf(p, remaining, L"%s", L"unhandled exception: ");
            ...
            ::MultiByteToWideChar(CP_ACP, 0, e.what(), -1, p,
            ...
            _snwprintf(p, remaining, L"%s", L"unhandled exception");
        }
    ...
    LONG WINAPI unhandled_exception_handler(LPEXCEPTION_POINTERS ep) noexcept
    {
        ...
        const auto n = GetModuleFileNameW(GetModuleHandleW(nullptr), exception_message,
