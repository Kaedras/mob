...
namespace mob {
    ...
    url::url(const char* p) : s_(p) {}

    url::url(std::string s) : s_(std::move(s)) {}
    ...
    void curl_downloader::run()
    {
        ...
        const std::string ua = "ModOrganizer's " + mob_version() + " " + curl_version();
        ...
        if (r == CURLE_OK) {
            ...
            if (h == 200) {
                ...
                cx_.trace(context::net, "curl: http 200 {}, transferred {} bytes", url_,
                          bytes_);
            ...
            cx_.error(context::net, "curl: {}, {} {}", curl_easy_strerror(r),
                      trim_copy(error_buffer), url_);
        }
    }

    size_t curl_downloader::on_write_static(char* ptr, size_t size, size_t nmemb,
                                            void* user) noexcept
    {
    ...
    int curl_downloader::on_progress_static(void* user, double, double, double,
                                            double) noexcept
    {
        auto* self = static_cast<curl_downloader*>(user);
    ...
    int curl_downloader::on_xfer_static(void* user, curl_off_t, curl_off_t, curl_off_t,
                                        curl_off_t) noexcept
    {
        auto* self = static_cast<curl_downloader*>(user);
    ...
    int curl_downloader::on_debug_static(CURL*, curl_infotype type, char* data,
    ...
    bool a_bit_too_much(std::string_view s)
    {
        static const char* strings[] = {
            "schannel: encrypted data", "schannel: encrypted cached",
            "schannel: decrypted data", "schannel: decrypted cached",
            "schannel: client wants",   "schannel: failed to decrypt data",
            "schannel: schannel_recv",  "schannel: Curl_read_plain"};
