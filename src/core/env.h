...
namespace mob {
    ...
    class env {
    public:
        using map = std::map<nativeString, nativeString>;
        ...
        // sets k=v
        //
        env& set(nativeString k, nativeString v, flags f = replace);
        env& set(nativeString k, nativeString v, flags f, nativeString separator);
#ifdef _WIN32
        env& set(std::string_view k, std::string_view v, flags f = replace);
        env& set(std::string_view k, std::string_view v, flags f,
    ...
    private:
        ...
        //
        nativeString* find(nativeStringView name);
        const nativeString* find(nativeStringView name) const;

        // called by set(), sets the value in the map
        //
        void set_impl(nativeString k, nativeString v, flags f, nativeString separator);
    ...
    private:
        ...
        //
        static std::optional<nativeString> get_impl(const std::string& k);
    };
